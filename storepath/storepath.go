// Package storepath 落盘路径派生——作品目录段与 store 文件名的唯一权威形态。
//
// 落盘布局（relPath，workDir 相对、正斜杠）：
//
//	store/resource/{siteKey}_{siteWorkId 派生段}/{role}_{seq 三位零填充}{ext}
//	示例：store/resource/pixiv_128937464/image_000.jpg
//	      store/resource/bilibili_BV1xx411c7mD_4538792/videoTrack_000.mp4
//
// "store/resource/" 前缀属主程序库内布局，不在本包——本包只产出目录段与文件段，
// 完整 relPath 由主程序侧 path.Join 组合；插件可引用本包推导兄弟文件名（如
