...
func (c *taskHandlerServiceClient) Resume(ctx context.Context, in *TaskResParamMessage, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StreamChunk], error) {
	...
	x := &grpc.GenericClientStream[TaskResParamMessage, StreamChunk]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
func _TaskHandlerService_Resume_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(TaskResParamMessage)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TaskHandlerServiceServer).Resume(m, &grpc.GenericServerStream[TaskResParamMessage, StreamChunk]{ServerStream: stream})
}
